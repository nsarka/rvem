...
class Memory:
  ...
  def read(self, addr, sz):
    ...
    r = self.mem[page][offset_in_begin_page:(offset_in_begin_page+begin_sz)]
    sz -= begin_sz
  ...
  def write(self, addr, data):
    ...
    if page not in self.mem:
      self.mem[page] = b'\x00'*self.page_size
    self.mem[page] = self.mem[page][:offset_in_begin_page] + data[:begin_sz] + self.mem[page][(offset_in_begin_page + begin_sz):]
    sz -= begin_sz
    data = data[begin_sz:]
...
def syscall(s, a0=0, a1=0, a2=0, a3=0, a4=0, a5=0):
  ...
  if s == Syscall.SYS_close:
    ...
    raise Exception("What is the system call isatty from puts.c in newlib? It didnt seem to be called but here we are. I added SYS_isatty = -1 just so it can be caught here")
...
def step():
  ...
  if opcode == Ops.SYSTEM:
    ...
    if sys_imm == 0:
      ...
      ret = syscall(Syscall(syscall_num), regfile.get("a0"), regfile.get("a1"), regfile.get("a2"), regfile.get("a3"), regfile.get("a4"), regfile.get("a5"))
      regfile.set("a0", ret)
    elif sys_imm == 1:
  ...
  for z in range(1):
    ...
    with open(x, 'rb') as f:
      ...
      #with open("test-cache/%s" % x.split("/")[-1], "wb") as g:
        #g.write(b'\n'.join([binascii.hexlify(memory[i:i+4][::-1]) for i in range(0,len(memory),4)]))
        #g.write(b'\n'.join([binascii.hexlify(memory[i:i+1]) for i in range(0,len(memory))]))
      print("Entry is ", hex(e.header.e_entry))
